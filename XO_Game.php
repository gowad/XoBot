...
function processMessage($message) {
  ...
  if (isset($message['text'])) {
    ...
    if (strpos($text, "/start") === 0) {
      apiRequestJson("sendMessage", array('chat_id' => $chat_id, "text" => "مرحبا بك 😃🖐 في بوت لعبة xo
...
function inlineMessage($inline){
 ...
 apiRequest("answerInlineQuery",array("inline_query_id"=>$id,"results"=>array(array("type"=>"article","id"=>$query,"title"=>"لعبه xo","input_message_content"=>array("message_text"=>"لعبة xo / اضغط على الزر للعب 👇","parse_mode"=>"HTML","disable_web_page_preview"=>false),
     "reply_markup"=>array(
         "inline_keyboard"=>array(
       array(array("text"=>"بدء اللعبه ❗️","callback_data"=>"play_".$chat_id))
   )
  )
 ))));
 exit;
...
function callbackMessage($callback){
   ...
   if(strpos($data, "play") === 0){
    $data=explode("_",$data);
    if($data[1]==$pv_id){
     apiRequest("answerCallbackQuery",array('callback_query_id'=>$callback_id,'text'=>"انتظر اللاعب الثاني ❌",'show_alert'=>false));
        exit;
    }
    else{
    ...
    apiRequest("editMessageText",array("inline_message_id"=>$message_id,"text"=>"هناك لاعب غادر اللعبه 🚶."," reply_markup"=>array(
   "inline_keyboard"=>$Tab 
    )));  
    exit;
   }
   ...
   "inline_keyboard"=>$Tab 
    ...
    $NextEmoji="❌";
     ...
     if($Tab[$i][$j]["text"]!=" ") apiRequest("answerCallbackQuery",array('callback_query_id'=>$callback_id,'text'=>"لا يمكنك الاختيار ❎",'show_alert'=>false));
     else{
      $Tab[$i][$j]["text"]=$Emoji;
                  //
      $n=0;
      ...
      $n=0;
       ...
       for($jj=0;$jj<3;$jj++){
        ...
        $n++;
         ...
         apiRequest("editMessageText",array("inline_message_id"=>$message_id,"text"=>"اللاعب الاول:$P1Name(❌)\nاللاعب الثاني:$P2Name(⭕️)\n\nلا يوجد رابح 😃 (تعادل)🏳","reply_markup"=>array(
                   "inline_keyboard"=>$Tab 
               )));  
         exit;
        }
      ...
      $Tab[3][0]["text"]="مغادرة اللعبه 😢";
               $Tab[3][0]["callback_data"]="Left";   
      //Tab
      
      $NextTurn=getChat($NextTurn);
            apiRequest("editMessageText",array("inline_message_id"=>$message_id,"text"=>"اللاعب الاول:$P1Name(❌)\nاللاعب الثاني:$P2Name(⭕️)\n\n دور اللاعب :  $NextTurn($NextEmoji) .","reply_markup"=>array(
                   "inline_keyboard"=>$Tab 
               )));
         exit;
        }
      ...
      apiRequest("answerCallbackQuery",array('callback_query_id'=>$callback_id,'text'=>"ليس دورك.",'show_alert'=>false));
   exit;
  }
 }
